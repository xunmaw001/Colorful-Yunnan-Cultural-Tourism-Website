...
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 导游
            //级联表
            DaoyouEntity daoyou = daoyouService.selectById(forum.getDaoyouId());
            if(daoyou != null){
            BeanUtils.copyProperties( daoyou , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yonghuId"
, "daoyouId"
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setDaoyouId(daoyou.getId());
            }
            //级联表 管理员
            //管理员表做额外的处理
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setDaoyouId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<ForumEntity> queryWrapper = new EntityWrapper<ForumEntity>()
            ...
            .eq("daoyou_id", forum.getDaoyouId())
...
//                            forumEntity.setYonghuId(Integer.valueOf(data.get(0)));   //游客 要改的
//                            forumEntity.setDaoyouId(Integer.valueOf(data.get(0)));   //导游 要改的
        ...
        ForumEntity forum = forumService.selectById(id);
            if(forum !=null){
                ...
                //级联表
                    DaoyouEntity daoyou = daoyouService.selectById(forum.getDaoyouId());
                if(daoyou != null){
                    BeanUtils.copyProperties( daoyou , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setDaoyouId(daoyou.getId());
                }
                    UsersEntity users = usersService.selectById(forum.getUsersId());
                if(users != null){
