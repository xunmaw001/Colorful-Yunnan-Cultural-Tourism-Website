        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('jiudianYuyue','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/qcybwenhualvyou/upload/jiudianYuyueMuBan.xls"
                        >批量导入酒店预定数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('jiudianYuyue','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="jiudianYuyueUploadSuccess"
                                :on-error="jiudianYuyueUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('jiudianYuyue','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入酒店预定数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jiudianYuyue','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jiudianYuyue.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="jiudianYuyueTime"
                                   header-align="center"
                                   label="预定时间">
                        <template slot-scope="scope">
                            {{scope.row.jiudianYuyueTime}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jiudianYuyueNumber"
                                      header-align="center"
                                      label="预定天数">
                        <template slot-scope="scope">
                            {{scope.row.jiudianYuyueNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('jiudianYuyue','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jiudianYuyue','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('jiudianYuyue','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                yonghuId : null,
                jiudianId : null,
                jiudianYuyueTime : null,
                jiudianYuyueNumber : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '邮箱': 'yonghuEmail',
                //本表字段
                     '预定时间': "jiudianYuyueTime",
                     '预定天数': "jiudianYuyueNumber",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'jiudian_yuyue',//当前表表名,
                        sumColum : 'jiudian_yuyue_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'jiudian_yuyue_name',//分组字符串字段
                        // types : 'jiudian_yuyue_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "jiudian_yuyue_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['jiudianYuyueDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "jiudianYuyue/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            jiudianYuyueUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jiudianYuyue/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "jiudianYuyue/update",
                        method: "post",
                        data: {
                            id:id,
//                            jiudianYuyueTypes:jiudianYuyueTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
